...
graph = gs.import_onnx(onnx.load("D:\\AI\\YOLO\\yolov8-main\\runs\\detect\\train3\\weights\\best.onnx"))


# 添加计算类别概率的结点

# input
origin_output = [node for node in graph.nodes if node.name == "Concat_291"][0]  # Concat_307是output的输入结点，不同的模型需要修改
print(origin_output.outputs)

output_t = gs.Variable(name="output_t",shape=(1,8400,11),dtype=np.float32)
...
ends_conf = gs.Constant("ends_conf",values=np.array([1,8400,11],dtype=np.int64))
...
label_conf_0 = gs.Variable(name='label_conf_0',shape=(1,8400,7),dtype=np.float32)
...
label_conf = gs.Variable(name='label_conf',shape=(1,8400,7),dtype=np.float32)   #这里要更改自己的类别
...
onnx.save(gs.export_onnx(graph),"D:\\AI\\YOLO\\yolov8-main\\runs\\detect\\train3\\weights\\best_1.onnx")
